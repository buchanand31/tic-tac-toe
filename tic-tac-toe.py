...
def play_game_level_2(board):
    draw_board(board)
    if char == 'O':
        print("Wait while I think...")
        while not game_over(board):
            select_space(board, minimax(board, True)[1], 'X')
            draw_board(board)
            if not game_over(board):
                value = int(input('Pick a value from: ' + str(legal_moves(board)) + '\n'))
                select_space(board, value, 'O')
                draw_board(board)
    else:
        while not game_over(board):
            value = int(input("Pick a value from: " + str(legal_moves(board)) + "\n"))
            select_space(board, value, 'X')
            draw_board(board)
            if not game_over(board):
